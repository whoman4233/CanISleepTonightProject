...
//////////////////////////////////////////////////////
	...
	#if defined(MK_LIGHT_BANDED)
		#define LIGHT_STYLE_RAW_2D_ADDITIONAL(value, atten, threshold, smoothnessMin, smoothnessMax, ramp, samplerRamp) value = BandingAdditional(value, _LightBands, smoothnessMin, smoothnessMax, threshold, _LightBandsScale)
	#elif defined(MK_LIGHT_CEL)
		#define LIGHT_STYLE_RAW_2D_ADDITIONAL(value, atten, threshold, smoothnessMin, smoothnessMax, ramp, samplerRamp) value = CelAdditional(threshold, smoothnessMin, smoothnessMax, value)
	#elif defined(MK_LIGHT_RAMP)
		#define LIGHT_STYLE_RAW_2D_ADDITIONAL(value, atten, threshold, smoothnessMin, smoothnessMax, ramp, samplerRamp) value = SampleRamp2D(PASS_TEXTURE_2D(ramp, samplerRamp), half2(value, atten)).r
	#else //MK_LIGHT_BUILTIN
		#define LIGHT_STYLE_RAW_2D_ADDITIONAL(value, atten, threshold, smoothnessMin, smoothnessMax, ramp, samplerRamp) value = max(0.0, value)
	#endif
	...
	MKLightData ComputeLightData(in MKLight light, in MKSurfaceData surfaceData)
	{
		...
		MKLight ComputeMainLight(in MKSurfaceData surfaceData, inout VertexOutputLight vertexOutputLight)
		{
			...
			INITIALIZE_STRUCT(MKLight, mkLight);
				...
				//lightdirection and attenuation
					...
					mkLight.distanceAttenuation = lerp(CelFalloff(0.0h, _AdditionalLightsFalloff, distanceAtten) * 0.5h, distanceAtten, _AdditionalLightsFalloff);
				...
				mkLight.shadowAttenuation = UNITY_SHADOW_ATTENUATION(vertexOutputLight, surfaceData.positionWorld);
					...
					#endif
					mkLight.shadowAttenuation = atten;
				#endif

				mkLight.attenuation = mkLight.shadowAttenuation * mkLight.distanceAttenuation;
		...
		MKLight ComputeAdditionalLight(int index, in MKSurfaceData surfaceData, inout VertexOutputLight vertexOutputLight)
		{
				...
				#endif
				mkLight.distanceAttenuation = lerp(CelFalloff(0, _AdditionalLightsFalloff, light.distanceAttenuation) * 0.5h, light.distanceAttenuation, _AdditionalLightsFalloff);
				//mkLight.distanceAttenuation = light.distanceAttenuation > 0.01;
				//mkLight.distanceAttenuation = smoothstep(0.001, 1, light.distanceAttenuation);
				//mkLight.distanceAttenuation = light.distanceAttenuation;

				mkLight.color = light.color;
				mkLight.radiometricColor = mkLight.color * PI;
				mkLight.attenuation = mkLight.distanceAttenuation * light.shadowAttenuation;
				mkLight.dirWorld = light.direction;
	...
	*/
	...
	// Lighting Direct
	/////////////////////////////////////////////////////////////////////////////////////////////
	inline half MKLightingDiffuse(inout Surface surface, in MKSurfaceData surfaceData, in MKPBSData pbsData, inout MKLight light, in MKLightData lightData, in half threshold)
	{
		//mkLight.distanceAttenuation = lerp(smoothstep(0, _AdditionalLightsFalloff, light.distanceAttenuation), light.distanceAttenuation, _AdditionalLightsFalloff);
		#ifdef MK_LIT
			half diffuse;
			...
			diffuse = lerp(diffuse, HalfWrap(diffuse, 0.5), step(HALF_MIN, _WrappedLighting));
			...
			//Lighting could be optimized by combining every component (diffuse, specular, lightTransmission, indirect/direct), may break gooch
				...
				LIGHT_STYLE_RAW_2D(light.shadowAttenuation, 1, threshold, _DiffuseSmoothness * 0.5, _DiffuseSmoothness * 0.5, _DiffuseRamp, SAMPLER_CLAMPED_MAIN);
				ARTISTIC_RAW_SHADOW_MASKED(light.shadowAttenuation, saturate(1.0 - light.shadowAttenuation), light.distanceAttenuation);
			#endif

			LIGHT_STYLE_RAW_2D(diffuse, light.distanceAttenuation, threshold, _DiffuseSmoothness * 0.5, _DiffuseSmoothness * 0.5, _DiffuseRamp, SAMPLER_CLAMPED_MAIN);
	...
	//URP lights has limitations regarding AA, different sampling is required when AA is enabled
	inline half MKLightingDiffuseAdditional(inout Surface surface, in MKSurfaceData surfaceData, in MKPBSData pbsData, inout MKLight light, in MKLightData lightData, in half threshold)
	{
		//mkLight.distanceAttenuation = lerp(smoothstep(0, _AdditionalLightsFalloff, light.distanceAttenuation), light.distanceAttenuation, _AdditionalLightsFalloff);
		#ifdef MK_LIT
			half diffuse;
			...
			//Lighting could be optimized by combining every component (diffuse, specular, lightTransmission, indirect/direct), may break gooch
			#ifdef MK_STYLIZE_SYSTEM_SHADOWS
				LIGHT_STYLE_RAW_2D_ADDITIONAL(light.shadowAttenuation, 1, threshold, _DiffuseSmoothness * 0.5, _DiffuseSmoothness * 0.5, _DiffuseRamp, SAMPLER_CLAMPED_MAIN);
				ARTISTIC_RAW_SHADOW_MASKED(light.shadowAttenuation, saturate(1.0 - light.shadowAttenuation), light.distanceAttenuation);
			#endif

			LIGHT_STYLE_RAW_2D_ADDITIONAL(diffuse, light.distanceAttenuation, threshold, _DiffuseSmoothness * 0.5, _DiffuseSmoothness * 0.5, _DiffuseRamp, SAMPLER_CLAMPED_MAIN);
	...
	{
		#ifdef MK_LIT
			half diffuseRaw = MKLightingDiffuseAdditional(surface, surfaceData, pbsData, light, lightData, _AdditionalLightsThreshold);
	...
	inline void LightingDirect(inout Surface surface, in MKSurfaceData surfaceData, in MKPBSData pbsData, in MKLight light, inout MKLightData lightData, out half4 finalLightColor)
	{
			...
			#ifdef MK_LEGACY_RP
				#ifdef USING_DIRECTIONAL_LIGHT
					half diffuseRaw = MKLightingDiffuse(surface, surfaceData, pbsData, light, lightData, _LightThreshold);
				#else
					half diffuseRaw = MKLightingDiffuseAdditional(surface, surfaceData, pbsData, light, lightData, _AdditionalLightsThreshold);
				#endif
			#else
