	...
	inline float ComputeLinearDepthToEyeDepth(float eyeDepth)
{
	...
	inline void MixAlbedoDetail(inout half3 albedo, in half4 detail, in int mode)
	{
		if(mode < 1)
	...
	inline half CelAdditional(half threshold, half smoothnessMin, half smoothnessMax, half value)
	{
		return smoothstep(threshold - smoothnessMin, threshold + smoothnessMax, value);
	}

	inline half CelFalloff(half threshold, half smoothness, half value)
	{
		return smoothstep(threshold, threshold + smoothness, value);
	}
	...
	{	
		#if defined(MK_LEGACY_BANDED_LIGHTING)
			levels--;
		...
		#elif defined(MK_BADED_LIGHTING_V2)
			levels--;
			...
			half stepSize = 1.0 / (levels - 1);

			v = Cel(threshold, fade, fade, v);

			half currentStep = floor(v / stepSize) * stepSize;
			half nextStep = currentStep + stepSize;

			half t = (v - currentStep) / stepSize;
			t = smoothstep(threshold - smoothnessMin, threshold + smoothnessMax, t);

			return lerp(currentStep, nextStep, t);
		#endif
	}

	inline half BandingAdditional(half v, half levels, half smoothnessMin, half smoothnessMax, half threshold, half fade)
	{	
		fade *= 0.5h;
		
		half stepSize = 1.0 / (levels - 1);

		v = CelAdditional(threshold, fade, fade, v);
	...
	{			
		//currently implemented as soft pattern, see repo for hard pattern prototype
			...
			return lerp(artisticClampMin, 1, value) * smoothstep(artistic - 0.125h - ddx, artistic + ddx, clamp(value, artisticClampMin, artisticClampMax));
		#else
			return lerp(artisticClampMin, 1, value) * smoothstep(artistic - 0.125h, artistic, clamp(value, artisticClampMin, artisticClampMax));
		#endif
	}
