...
//////////////////////////////////////////////////////
	...
	)
	{
		...
		surfaceData.svPositionClip = svPositionClip;
		...
		#endif
		#if defined(MK_TCM) || defined(MK_TCD)
			surfaceData.secondaryUV = baseUV.zw;
		#endif
		...
		#endif
		#if defined(MK_TCD)
			surfaceData.detailUV = surfaceData.secondaryUV * _DetailMap_ST.xy + _DetailMap_ST.zw;
		#endif
			...
			surfaceData.artisticUV.xy += lerp(0, Stutter(MK_TIME.y, _ArtisticFrequency), step(HALF_MIN, _ArtisticFrequency - 1.0h));
		#endif
	...
	inline Surface InitSurface(inout MKSurfaceData surfaceData, DECLARE_SAMPLER_2D_ARGS(samp2D), in half4 albedoTint, in float4 pC)
	{
		...
		//add detail
			...
			MixAlbedoDetail(surface.albedo, surface.detail, _DetailBlend);
		#endif
